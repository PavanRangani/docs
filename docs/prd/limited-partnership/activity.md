...
- For `Net` or `Net with Separate call` type activity, `Call amount` or `Cash Distribution` or `Distribution amount` can't be changed in such a way that already created task is changed from `Capital call` to `Distribution` or vice versa.
